...
def plot_histogram():
    ...
    plt.errorbar(frame['g_dsr'], np.mean(g_dsa_matrix, axis=0),yerr=np.std(g_dsa_matrix, axis=0), ecolor= 'r', label=r"$\Delta g_{dsa}$", fmt='none')
    plt.legend()
    #plt.title(r"$alpha$:%s,$beta$:%s,$g_{dsa}$:%s,$g_{dsr}$:%s" % (frame['expected'][0], frame['expected'][1], frame['expected'][2], frame['expected'][3]))
    plt.xlabel(r'Error of $g_{dsr}$ [$\%$]')
    plt.ylabel(r'$\Delta [\%]$')
    plt.show()
    ...
    plt.errorbar(frame['g_dsa'], np.mean(g_dsr_matrix, axis=0),yerr=np.std(g_dsr_matrix, axis=0), ecolor= 'r', label=r"$\Delta g_{dsr}$", fmt='none')
    plt.legend()
    #plt.title(r"$alpha$:%s,$beta$:%s,$g_{dsa}$:%s,$g_{dsr}$:%s" % (frame['expected'][0], frame['expected'][1], frame['expected'][2], frame['expected'][3]))
    plt.xlabel(r'Error of $g_{dsa}$ [$\%$]')
    plt.ylabel(r'$\Delta [\%]$')
    plt.show()
