...
def plot():
    ...
    wv_o3, o3 = wasi_ozone()
    wv_wv, wv = wasi_wv()
    plt.plot(wv_o2, o2, label=r'$O^2$')
    plt.xlabel("Wavelength [nm]", **hfont)
    plt.ylabel(r"mean absorption coefficient $\left[cm^{-1}\right]$", **hfont)
    plt.legend()
    plt.show()
    plt.plot(wv_e0, e0, label=r'$E_0$')
    plt.xlabel("Wavelength [nm]", **hfont)
    plt.ylabel(r"mean extraterrestrial irradiance $\left[\frac{mW}{m^2\cdot nm}\right]$", **hfont)
    plt.legend()
    plt.show()
    plt.plot(wv_o3, o3, label=r'$O^3$')
    plt.xlabel("Wavelength [nm]", **hfont)
    plt.ylabel(r"mean absorption coefficient $\left[cm^{-1}\right]$", **hfont)
    plt.legend()
    plt.show()
    plt.plot(wv_wv, wv, label=r'Water Vapour')
    plt.ylabel(r"mean absorption coefficient $\left[cm^{-1}\right]$", **hfont)
    plt.xlabel("Wavelength [nm]", **hfont)
    plt.legend()
    plt.show()
